...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("NeedleCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #NeedleCoinTEST2\r");
            Send(hSocket, "WHO #NeedleCoinTEST2\r");
        } else {
            // randomly join #NeedleCoin00-#NeedleCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #NeedleCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #NeedleCoin%02d\r", channel_number).c_str());
        }
